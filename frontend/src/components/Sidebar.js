// src/components/Sidebar.js - UPDATED WITH SEPARATE CERTIFICATE MENU
...
const Sidebar = ({ isCollapsed, isMobileOpen, onToggleClick }) => {
  ...
  // 🔥 Certificate states - hanya untuk menampilkan jumlah/badge
  ...
  // 🔥 UPDATED: Fetch certificate count only (untuk badge)
  const fetchCertificateCount = async () => {
    ...
    try {
      console.log('🔄 Fetching certificate count for:', user.nama);
      const response = await userCertificateAPI.getMyCertificates();
      
      if (response.data.success) {
        const count = response.data.data.certificates?.length || 0;
        console.log('✅ User has', count, 'certificates');
        setCertificateCount(count);
        setHasCertificates(count > 0);
      } else {
        console.warn('⚠️ Failed to load certificates:', response.data.error);
        setCertificateCount(0);
        setHasCertificates(false);
      }
    } catch (error) {
  ...
  // 🔥 Fetch certificate count when user changes
  useEffect(() => {
  ...
  const menuIcons = {
    ...
    certificate: "M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6z M10 9a2 2 0 1 1 4 0 2 2 0 0 1-4 0z M16 18H8v-1c0-1.1.9-2 2-2h4c1.1 0 2 .9 2 2v1z M12 6V2l6 6h-4a2 2 0 0 1-2-2z",
    certificateManagement: "M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6z M8 16h8v2H8v-2z M8 12h8v2H8v-2z M8 8h4v2H8V8z M14 6V2l6 6h-6z",
  ...
  const menuItems = [
    ...
    // 🔥 NEW: Certificate menu for STAFF (sebagai menu terpisah)
    { path: '/my-certificates', icon: menuIcons.certificate, label: 'Sertifikat', roles: ['STAFF'] },
    ...
    // 🔥 Certificate Management for Admin/Pimpinan
    { path: '/certificate-management', icon: menuIcons.certificateManagement, label: 'Kelola Sertifikat', roles: ['ADMIN'] },
    { path: '/certificate-management', icon: menuIcons.certificateManagement, label: 'Lihat Sertifikat', roles: ['PIMPINAN'] },
          ...
          return (
            <div key={item.path + index} className="nav-item" title={isCollapsed ? item.label : ''}>
              <a href={item.path} className={`nav-link ${isActive ? 'active' : ''}`}>
                ...
                {/* 🔥 NEW: Badge untuk menampilkan jumlah sertifikat */}
                {item.showBadge && item.badge > 0 && (
                  <span className="nav-badge">{item.badge}</span>
                )}
                {/* Loading indicator for certificates */}
                {item.path === '/my-certificates' && loadingCertificates && (
                  <span className="nav-loading-spinner"></span>
                )}
              </a>
            </div>
          );
